...
UPSTREAM_REPLACE_NAMES = {
    "web_enterprise", "web_gantt", "web_cohort", "web_grid", "web_map", "documents",
}
ENTERPRISE_DEPENDS = {
    "account_reports", "crm_enterprise", "documents", "mrp_workorder", "quality",
    "sale_subscription", "web_enterprise", "web_gantt", "web_grid", "web_map",
}
...
def read_depends(module_path: Path) -> list[str]:
    return [d for d in read_manifest(module_path).get("depends", []) if isinstance(d, str)]
...
def classify_module(name: str, module_path: Path) -> dict:
    data = read_manifest(module_path)
    author = str(data.get("author", ""))
    license_ = str(data.get("license", ""))
    depends = read_depends(module_path)
    flags = []
    if name in UPSTREAM_REPLACE_NAMES or license_.upper() == "OEEL-1":
        flags.append("replace_upstream")
    if re.search(r"\bOCA\b", author):
        flags.append("wait_oca_target")
    if any(d in ENTERPRISE_DEPENDS or d.endswith("_enterprise") for d in depends):
        flags.append("enterprise_required")
    if data.get("auto_install"):
        flags.append("auto_install_risk")
    return {
        "class": flags[0] if flags else "custom",
        "flags": flags,
        "depends": depends,
        "license": license_,
        "author": author,
...
def stage_modules(modules: dict[str, Path], names: list[str], stage_dir: Path) -> Path:
    marker = stage_dir / ".odoo_ai_stage"
    if stage_dir.exists():
        if marker.exists():
            shutil.rmtree(stage_dir)
        elif any(stage_dir.iterdir()):
            raise RuntimeError(f"refuse to clean non-stage dir: {stage_dir}")
    stage_dir.mkdir(parents=True, exist_ok=True)
    marker.write_text("generated by migrate_all.py; safe to delete\n")
    for name in names:
        shutil.copytree(modules[name], stage_dir / name, symlinks=True)
    (stage_dir / "staged_modules.txt").write_text(",".join(names))
    return stage_dir
...
def run_fleet(roots: list[Path], manifest_path: Path, out_dir: Path,
    ...
    source_sweep = _load_sibling("source_sweep")
    manifest = json.loads(manifest_path.read_text())
    modules = discover_modules(roots)
    depends = {n: read_depends(p) for n, p in modules.items()}
    classes = {n: classify_module(n, p) for n, p in modules.items()}
    ...
    for name in order:
        ...
        per_module[name]["classification"] = classes[name]
    ...
    fleet = {
        "meta": {
            ...
            "classification_note": ("Static manifest classifier. replace_upstream/OCA/"
                                    "enterprise flags are routing hints, not proof."),
            "source_sweep_summary": sweep["summary"],
        },
        "order": order,
        "totals": totals,
        "modules": per_module,
    ...
    (out_dir / "source_sweep.json").write_text(json.dumps(sweep, indent=2, ensure_ascii=False))
    return fleet
...
def main() -> int:
    ...
    ap.add_argument("--verify-batch", action="store_true",
                    help="run one upgrade_verify.py batch over the portable set")
    ap.add_argument("--stage-dir", type=Path,
                    help="copy only the portable custom modules into this clean addons dir")
    ...
    if args.stage_dir:
        stage_modules(modules, portable, args.stage_dir)
        print(f"staged clean addons dir: {args.stage_dir}")
    verify_batch_rc = 0
    if args.verify_batch:
        cmd = [sys.executable, str(HERE / "upgrade_verify.py"),
               "--modules-file", str(args.out / "install_set.txt"),
               "--out", str(args.out / "_batch"), "--db", args.db]
        for flag, val in (("--docker-compose", args.docker_compose),
                          ("--odoo-bin", args.odoo_bin),
                          ("--addons-path", args.addons_path)):
            if val:
                cmd += [flag, val]
        verify_batch_rc = subprocess.run(cmd).returncode
        fleet["meta"]["verify_batch_rc"] = verify_batch_rc
        (args.out / "fleet.json").write_text(json.dumps(fleet, indent=2, ensure_ascii=False))
