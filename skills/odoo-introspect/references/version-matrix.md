...
| User groups field | `res.users.groups_id` | **`res.users.group_ids`** (`groups_id` renamed). `ir.rule.groups` is **unchanged** | **v19** | **script** (Layer G reads the user's groups; resolves the field name at runtime) |
...
- **`res.users.groups_id` → `group_ids` (v19).** The user-groups relation was renamed (`ir.rule.groups` was **not**). Old code reading `user.groups_id` raises `AttributeError` on 19. Layer G (`security_sim.py`) resolves the field name from the registry at runtime, so it runs unchanged on 17 → 19. → `odoo-security`, `odoo-review`.
- **The introspection scripts themselves are v19-safe** — they read the registry (`fields_get`, `ir.model.access`, `ir.rule`, `get_view`) and do **not** call the deprecated `read_group` / `check_access_rights` / `name_get`; the one renamed field they touch (`res.users.group_ids`) is resolved at runtime, so the engine runs unchanged on 17 → 19. The version drift lives in the *guidance*, not the tooling.
